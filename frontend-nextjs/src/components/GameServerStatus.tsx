...
export default function GameServerStatus({ status, phase, pending }: {
  ...
  return <div className="w-full rounded border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
    ...
    <GameServerConnect />
    <p className="mt-1">Isınma: .guns · Hazır: .ready · Duraklat: .pause · Devam: iki takım da .unpause</p>
    {message && <p className="mt-2">{message}</p>}
    {status && <div className="mt-2 space-y-1">
      <p>{status.map} · {status.humans} oyuncu · {status.preparing ? 'Demo kontrolü' : status.paused ? 'Maç duraklatıldı' : status.live ? 'Maç canlı' : 'Süresiz ısınma'}</p>

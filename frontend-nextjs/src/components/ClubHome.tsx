  ...
  return <div className="club-home">
    ...
    <section className="club-night-grid" aria-label="Maç gecesi hazırlığı">
      <div className="club-night-card">
        {clubVersion !== "original" && <div className="club-night-art" aria-hidden="true" />}
    ...
    <section className={clubVersion === "original" ? undefined : "club-competitions-section"} aria-labelledby="club-competitions-title"><div className="club-section-heading"><h2 id="club-competitions-title">Hesaplaşma vakti</h2><Link prefetch={false} href="/batak-domination">Domination haritası <ArrowUpRight size={15} /></Link></div>
      <div className="club-competition-grid">{competitions.map(({ href, name, tag, description, icon: Icon, className }) => <Link prefetch={false} href={href} className={`club-competition-card ${className}`} key={href}>{clubVersion === "original" ? <div className="club-card-top"><span className="club-competition-icon"><Icon size={27} strokeWidth={1.5} /></span><ArrowUpRight size={20} /></div> : <div className="club-competition-art" aria-hidden="true"><span className="club-emblem-orbit" /><span className="club-emblem"><Icon size={72} strokeWidth={1.2} /></span>{clubVersion === "panels" && <><span className="club-art-caption">CS BATAĞI / {tag}</span><ArrowUpRight size={20} /></>}</div>}<p className="club-eyebrow">{tag}</p><h3>{name}</h3><p>{description}</p><span className="club-competition-cta">Tabloyu görüntüle <ArrowRight size={16} /></span></Link>)}</div>
    </section>

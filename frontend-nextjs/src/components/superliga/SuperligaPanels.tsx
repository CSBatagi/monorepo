...
/**
 * Superliga puanlamasını kullanan sezonların (Superliga, Batak Mundial) ortak
 * gece yönetimi panelleri: kaptan atama, eksik maç sonucu, manuel gece ve
 * sezon ilerleme çubuğu. Gece verileri tarih bazlı ortak tablolarda tutulur;
 * sezonlar tarih aralığıyla ayrılır.
 */
...
export const CLEAR_ATTENDANCE_PASSWORD = process.env.NEXT_PUBLIC_CLEAR_ATTENDANCE_PASSWORD || 'osirikler';

type TeamKey = 'team1' | 'team2';
type SessionUser = ReturnType<typeof useSession>['user'];
type Scoring = SuperligaConfig['scoring'];
...
export function PositionChangeIndicator({ change }: { change?: 'up' | 'down' | 'same' | 'new' }) {
  switch (change) {
    case 'up':
...
const capitalize = (text: string) => text.charAt(0).toLocaleUpperCase('tr') + text.slice(1);
...
export function SeasonProgressBar({
  ...
  dates: string[];
  /** Label for one progress box, e.g. "maç" or "gece". */
  unit?: string;
}) {
  ...
  for (let i = 0; i < totalBoxes; i++) {
    ...
    squares.push(
      <button
        ...
        title={isPlayed ? `${capitalize(unit)} ${matchNum}: ${dates[i] || ''}` : `${capitalize(unit)} ${matchNum} (oynanmadı)`}
      >
        {matchNum}
      </button>,
    );
  ...
  const viewingText = selectedIndex === null || selectedIndex >= played ? `Güncel (${played} ${unit})` : `${capitalize(unit)} ${selectedIndex} sonrası`;

  return (
    <div className="flex flex-col items-center gap-2 py-3">
      <div className="flex flex-wrap justify-center gap-1 sm:gap-1.5">{squares}</div>
      <div className="text-xs text-gray-500">
        {played} / {total} {unit} oynandı · <span className="font-medium text-purple-600">{gamesLeft} {unit} kaldı</span>
      </div>
      <div className="text-xs text-gray-500">
...
export function NightMapsDetail({ entry }: { entry: SuperligaNightEntry }) {
...
export function MapOverridePanel({
  ...
  const scoring = scoringProp || DEFAULT_SUPERLIGA_SCORING;
...
export function ManualNightPanel({
  scoring: scoringProp,
  poolSteamIds,
  seasonStart,
  seasonEnd,
  ...
  scoring?: Scoring;
  /** Puan kazanabilecek oyuncular (lig kadrosu / kura katılımcıları). */
  poolSteamIds: string[];
  ...
  const dateAfterSeason = !!(date && seasonEnd && date > seasonEnd);
  ...
  return (
    <div className="space-y-4">
      <div className="rounded border border-slate-200 bg-slate-50 p-4 shadow-sm">
        ...
        {dateAfterSeason && !dateHasDemo && (
          <p className="mt-2 text-xs text-amber-600">⚠️ Bu tarih sezon bitişinden ({seasonEnd}) sonra; puan durumuna dahil edilmez.</p>
        )}
...
export function CaptainAssignmentPanel({
  availableDates,
  sonmacByDate,
  manualNights,
  captainsByDate,

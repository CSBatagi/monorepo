...
import { Check, ChevronLeft, ChevronRight, Search, RotateCcw } from 'lucide-react';
...
import EquipmentProgress, { EquipmentHelp } from './EquipmentProgress';
...
export default function EquipmentClient() {
  ...
  return <div className="equipment">
    ...
    {loading && !account ? <p role="status">Ekipman yükleniyor…</p> : account && <>
      <EquipmentProgress account={account} refresh={() => void refreshProgress()} busy={busy} selectTier={value => { setTier(value); setOffset(0); document.getElementById('equipment-catalog')?.scrollIntoView({ behavior: 'instant', block: 'start' }); }} />
      {state && <fieldset className="equipment-workspace" disabled={busy}>
        ...
        <div className="equipment-columns"><main id="equipment-catalog"><h2>Tarzını seç.</h2>
          ...
          <div className="equipment-filters"><label className="equipment-search"><Search size={17} /><input aria-label="Eşya ara" value={query} placeholder="Dragon Lore, Doppler, Crimson…" onChange={event => { setQuery(event.target.value); setOffset(0); }} /></label>{listing.weapons.length > 0 && <select aria-label="Silah modeli" value={weapon} onChange={event => { setWeapon(event.target.value); setOffset(0); }}><option value="">Tüm modeller</option>{listing.weapons.map(name => <option key={name}>{name}</option>)}</select>}<select aria-label="Eşya kademesi" value={tier} onChange={event => { setTier(event.target.value); setOffset(0); }}><option value="">Tüm kademeler</option>{account.tiers.map(t => <option key={t.tier} value={t.tier}>{t.label}</option>)}</select></div>
          ...
          <div className="equipment-grid" aria-busy={catalogLoading}>{!catalogLoading && listing.items.map(item => <button className="equipment-card" key={item.id} disabled={!item.teams.includes(team)} style={{ '--item-color': item.color } as React.CSSProperties} onClick={() => { const existing = selectedProfile?.items.find(i => i.id === item.id && i.team === team); setEditing(existing ? structuredClone(existing) : selectionFor(item, team)); setAttachment(null); }}>
            ...
            <span className={`equipment-access ${item.access.tier}`}>{item.access.label} · {isOwned(item) ? item.access.cost === 0 ? 'Ücretsiz' : 'Koleksiyonunda' : priceLabel(item)}</span>{!isOwned(item) && item.access.level > 1 && <small>{(account.progress?.level ?? 0) < item.access.level ? `Seviyen ${account.progress?.level ?? '—'} · ${item.access.level} gerekli` : canUnlock(item) ? 'Açmaya hazırsın' : `${Math.max(0, item.access.cost - (account.progress?.[item.access.currency] ?? 0))} jeton daha gerekli`}</small>}
          </button>)}</div>
        ...
        </main><aside className="equipment-loadout"><span className="equipment-eyebrow">{selectedProfile?.name}</span><h2>Senin setin.</h2><div className="equipment-team">{[2, 3].map(value => <button key={value} aria-pressed={team === value} className={team === value ? 'active' : ''} onClick={() => setTeam(value)}>{value === 2 ? 'T' : 'CT'}</button>)}</div><p className="equipment-hint">Her takım için ayrı seçim yapabilirsin. Müzik iki takımda da aynı çalar.</p>
          {currentItems.length === 0 && <p className="equipment-empty">Setin seni bekliyor.<br />Koleksiyondan bir eşya seç.</p>}
          ...
          <a className="equipment-hint" href="#equipment-help">Steam ekipmanı nasıl çalışır?</a>
    ...
    {editing && selectedItem && createPortal(<div className="equipment equipment-overlay" onClick={event => { if (event.target === event.currentTarget) setEditing(null); }}><section className="equipment-editor" role="dialog" aria-modal="true" aria-label="Eşyayı düzenle" onKeyDown={event => {
      ...
      {!isOwned(selectedItem) && <div className="equipment-unlock-info"><strong>{priceLabel(selectedItem)} · Kalıcı açılış</strong><p>{selectedItem.access.currency === 'premiumTokens' ? 'Özel ödül koleksiyonu. Sezon başarın veya katkın için yöneticinin verdiği premium jetonla seçebilirsin.' : `Seviye ${selectedItem.access.level} gerekli. Bir kez aç, her setinde kullan.`}</p><small>Senin seviyen: {account?.progress?.level ?? '—'} · Bakiyen: {account?.progress?.tokens ?? 0} jeton · {account?.progress?.premiumTokens ?? 0} premium</small><p>{(account?.progress?.level ?? 0) < selectedItem.access.level ? `Açmak için ${selectedItem.access.level}. seviyeye ulaşmalısın.` : !canUnlock(selectedItem) ? `${Math.max(0, selectedItem.access.cost - (account?.progress?.[selectedItem.access.currency] ?? 0))} ${selectedItem.access.currency === 'premiumTokens' ? 'premium jeton' : 'jeton'} daha gerekli.` : 'Açmaya hazırsın. Jeton yalnızca bir kez harcanır.'}</p>{!account?.steamId && <p>Önce Steam hesabınla giriş yap.</p>}</div>}

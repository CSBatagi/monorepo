...
export class Battlefield {
  ...
  private project(p: Vec3) {
    ...
    return { x: this.width * .54 + (x * Math.cos(c.roll) - y * Math.sin(c.roll)) * scale,
      y: this.height * .5 + (y * Math.cos(c.roll) + x * Math.sin(c.roll)) * scale, scale, depth };
  }
  ...
  draw(width: number, height: number, time: number, camera: Camera) {
    ...
    this.focal = Math.max(500, Math.min(950, width * .65));
    const ctx = this.ctx, mobile = width < 700; ctx.clearRect(0, 0, width, height);
    // Expanding, rolling smoke volumes moving away from the breach.
    for (let i = 0; i < (mobile ? 15 : 30); i++) {
    ...
    const shards = Array.from({ length: mobile ? 26 : 64 }, (_, i) => debrisAt(i, time)).sort((a, b) => b.z - a.z);

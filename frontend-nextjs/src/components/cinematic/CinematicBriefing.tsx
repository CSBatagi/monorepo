...
export default function CinematicBriefing() {
  ...
  return <div className="cinema-briefing">
    <section className="cinema-brief-panel cinema-attendance" aria-labelledby="cinema-attendance-title">
      ...
      {data.preview && <p className="cinema-preview-note">Örnek katılım · bu yerel incelemede kayıt yapılmaz.</p>}
      ...
      {user ? <form className="cinema-attendance-form" onSubmit={event => event.preventDefault()}>
        ...
        <select id="cinema-player" value={selected} disabled={saving || data.preview} onChange={event => selectPlayer(event.target.value)}><option value="">Önce adını seç</option>{players.map(player => <option value={player.steamId} key={player.steamId}>{player.name}</option>)}</select>
        <div className="cinema-attendance-choices">{choices.map(({ status, label, icon: Icon }) => <button key={status} type="button" data-status={status} aria-pressed={currentStatus === status} disabled={!selected || saving || loading || !!error || data.preview} onClick={() => void submit(status)}><Icon size={14} />{label}</button>)}</div>

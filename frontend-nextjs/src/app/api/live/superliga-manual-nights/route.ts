...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${BACKEND}/live/superliga-manual-nights?v=${v}`, { cache: 'no-store' });
...
async function proxyPost(path: string, body: any) {
  const res = await fetch(`${BACKEND}/live/superliga-manual-nights/${path}`, {
    method: 'POST',

  ...
  try {
    ...
    if (lastGeneratedData) {
      counts = {
        ...
        night_avg_periods: lastGeneratedData.night_avg_periods ? Object.keys(lastGeneratedData.night_avg_periods.data || {}).length : 0,
        ...
        sonmac_by_date_periods: lastGeneratedData.sonmac_by_date_periods ? Object.keys(lastGeneratedData.sonmac_by_date_periods.data || {}).length : 0,
...
if (!TEST_MODE) {
  ...
  async function runMigrations() {
    const migrations = [
      ...
      `INSERT INTO live_version (key, version) VALUES ('attendance', 0), ('team_picker', 0), ('mvp_votes', 0), ('batak_captains', 0), ('batak_super_kupa', 0), ('token_wars', 0), ('token_wars_captains', 0), ('superliga_captains', 0), ('superliga_map_overrides', 0), ('superliga_manual_nights', 0), ('mundial', 0) ON CONFLICT DO NOTHING`,
      ...
      `CREATE TABLE IF NOT EXISTS token_wars (id SERIAL PRIMARY KEY, actor_steam_id TEXT NOT NULL, target_steam_id TEXT NOT NULL, token_type TEXT NOT NULL, set_by_uid TEXT, set_by_name TEXT, set_at BIGINT, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`,
      // Superliga tables — puan bazlı tek liste lig, kaptan atamaları
      `CREATE TABLE IF NOT EXISTS superliga_captains (date TEXT NOT NULL, team_key TEXT NOT NULL, steam_id TEXT NOT NULL, steam_name TEXT, team_name TEXT, set_by_uid TEXT, set_by_name TEXT, set_at BIGINT, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), PRIMARY KEY (date, team_key))`,
      // Superliga manuel maç sonucu override'ları — eksik harita sonuçlarını elle eklemek için
      `CREATE TABLE IF NOT EXISTS superliga_map_overrides (date TEXT NOT NULL, map_name TEXT NOT NULL, team1_name TEXT, team1_score INT NOT NULL, team2_name TEXT, team2_score INT NOT NULL, set_by_uid TEXT, set_by_name TEXT, set_at BIGINT, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), PRIMARY KEY (date, map_name))`,
      // Superliga tamamen manuel geceler — demo/istatistik olmayan geceleri elle eklemek için (takım kadroları + harita skorları)
      `CREATE TABLE IF NOT EXISTS superliga_manual_nights (date TEXT PRIMARY KEY, team1_name TEXT, team2_name TEXT, team1_players JSONB NOT NULL DEFAULT '[]', team2_players JSONB NOT NULL DEFAULT '[]', maps JSONB NOT NULL DEFAULT '[]', set_by_uid TEXT, set_by_name TEXT, set_at BIGINT, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`,
      // Batak Mundial — kura sonucu ('draw') ve eleme maçları ('ko:<slot>'); gece puanlaması Superliga tablolarını kullanır
      `CREATE TABLE IF NOT EXISTS mundial_state (key TEXT PRIMARY KEY, value JSONB NOT NULL, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`,

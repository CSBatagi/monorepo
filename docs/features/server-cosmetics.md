...
## Personal equipment dashboard

The page leads with the member's profile, current level, XP progress, token balances and next tier. Clickable tier cards filter the catalogue and distinguish level requirements from currency requirements. Item dialogs show the member's level/balance and exactly what is missing. Levels belong to the player; weapons have unlock tiers, not separate XP levels. Detailed rewards, set instructions and Steam inventory coexistence are available in the Ekipman rehberi disclosures below the collection. Admin awards remain in that guide. Existing unlock/save/revision and Steam fallback behavior are unchanged.

...
- Both interfaces also offer **Sinematik deneyim**. This is an independent layout, accessible directly with `?ui=cinematic`. Its **Arayüz** menu (or the full navigation menu on mobile) returns to the four club variations or the classic interface without changing their saved palettes or version.
...
The cinematic home is a continuous four-chapter journey: Kulüp, Rekabet, Maç merkezi, and İstatistik. Native vertical scrolling and chapter anchors move a real Three.js camera around a Dust II–inspired breach, from an establishing shot to the opposing side, a close angle, and an overhead view. Suspended rubble, brass, and smoke move independently. Pointer movement adds subtle parallax. Native scrolling remains available for touch, keyboard, and long sections on small screens. Attendance follows the opening scene instead of obscuring it.
...
`CinematicScene.tsx` lazily loads Three.js only in cinematic mode. Geometry, material batching, scroll-scrubbed operator poses, rendering budgets, local asset provenance, and the read-only preview command are documented in [cinematic-3d.md](cinematic-3d.md). Phones start with fewer operators/effects, 30 fps, DPR at most 1, and an 850,000-pixel budget; desktop caps are 60 fps, DPR 1.5, and 1.9 million pixels. Expensive rendering drops resolution further. Hidden tabs and detail pages have no continuous scene loop. The existing 141,028-byte illustration is the no-WebGL/data-saver/static fallback; its provenance remains in [cinematic-artwork.md](cinematic-artwork.md).

The footer motion button pauses camera motion, particles, smoke, quotes, reveals, and smooth scrolling. Its preference is stored in `cs-batagi-cinema-motion`. System reduced-motion preferences disable motion on initial load and when changed. A separate scene button cycles automatic, light, and static modes, remembered in `cs-batagi-cinema-quality`. Cinematic mode uses a fixed dark palette and its own theme storage key so entering it cannot overwrite classic or club light/dark choices. Styling is isolated in `src/styles/cinematic.css` and `cinematic-3d.css`; existing table data and semantic status colors are retained.

...
2. **Client refresh**: Client components use the shared `useStatsRefresh` hook (`lib/useStatsRefresh.ts`) which calls `/api/stats/check` (incremental — only returns data when `updated: true`). The hook keeps the global backend `statsVersion` model but tracks the last received version separately for each mounted consumer (never in localStorage), but callers can pass dataset `keys` so the browser receives only the datasets that component can consume. The endpoint still persists the complete backend snapshot before filtering the public response. The hook provides `onData` (fresh data) and `onSettled` (always fires, for clearing loading state) callbacks.
...
- **Frontend polling**: `useLivePolling` hook polls every 3s with version-based 304 responses
  ...
  - Conditional reads require an exact version match, so a database restore with a lower version cannot strand clients on stale data.
  - Polling schedules the next request after completion, pauses in hidden tabs, and checks immediately on visibility/pageshow/online resume. Requests time out after 15s.
  - `refetch()` aborts/supersedes older reads and starts a forced read after the mutation; older responses cannot overwrite it.
  - Single-player attendance edits and resets commit data and version in one transaction, as bulk edits already do.
  - Each newly mounted stats consumer explicitly sends `lastKnownVersion=0` until it has received its own payload. Omitting it lets the proxy substitute a disk version, which does not prove that the browser has that data.

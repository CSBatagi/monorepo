...
2. **Worker on the game VM.** The workflow copies [`analyzer-maintenance.sh`](../../ops/cs2/analyzer-maintenance.sh) and `demo-analyzer.py` over SSH (IAP as fallback), installs the worker if its sha256 differs, waiting for a running analysis first, and restarts `csbatagi-analyzer`. A stopped VM is started for the install and stopped again once no analysis, match or player is on it; a busy VM is left running with a warning. The new worker also works with the previous backend.

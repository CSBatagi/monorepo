...
- **Backend** ([demoRoutes.js](../../backend/demoRoutes.js)) owns the `demo_files` table. It queues a demo automatically only when the recording ended normally (`map-ended`), the archive upload is verified, and the match ID belongs to a match the website created (its JSON exists in `CS2_MATCH_DIR`). That excludes bot simulations and console tests. Admins can queue anything else, including re-analysis with `--force`. A worker's "analyzed" report is accepted only if the database has the match: a `matches` row whose `demo_path` ends in the demo's file name (the path of the file the CLI analyzed), or else a `demos` row named after its stem; the CLI exits 0 even on failure, so the database is the proof. The path comes first because CS Demo Manager keeps the first name it saw for a checksum and the desktop app records every demo it lists, so a demo analyzed here can sit in `demos` under a member's file name (26 September 2026: the first xplay uploads were analyzed into the statistics but reported as "not in the database"). The same two links are applied to every listing, so such rows correct themselves. It also exits 0 when it skips a demo whose checksum is already in `matches` (analyzed from a desktop under another name) and when it rolls a failed insertion back, so when the row is missing the backend stores the CLI's own output as the error (the worker drops stack frames from it and logs the full output to the journal).
...
2. **Worker on the game VM.** The workflow copies [`analyzer-maintenance.sh`](../../ops/cs2/analyzer-maintenance.sh) and `demo-analyzer.py` over SSH through Identity-Aware Proxy (the game VM's firewall drops direct SSH; direct is tried only as a fallback), installs the worker if its sha256 differs, waiting for a running analysis first, and restarts `csbatagi-analyzer`. Every SSH call has a time limit, and a worker that could not be installed fails the run instead of ending green. It then writes the CLI's database login on the game VM (`/home/steam/.config/csdm/settings.json`: the backend VM's internal IP from `gcloud`, port 5432, `csdm_analyzer`, `csdm`, and the `CSDM_ANALYZER_PASSWORD` secret the role is created with) and logs in with it as the CLI would; a failed login fails the run. Dry runs on other branches print the current settings (never the password) and the login result. The deploy account needs `roles/iap.tunnelResourceAccessor` for this; without it the run fails with `4033: 'not authorized'` and prints the grant command. A stopped VM is started for the install and stopped again once no analysis, match or player is on it; a busy VM is left running with a warning. The new worker also works with the previous backend.
...
| Postgres role `csdm_analyzer` on the backend database | Not created until 26 September 2026: the repository ignores `*.sql`, so [`backend/migrations/csdm_analyzer_role.sql`](../../backend/migrations/csdm_analyzer_role.sql) was never committed and the deploy step never ran it (the first real analysis then failed inside `csdm` with "Error while migrating database"). The file is now committed (`!backend/migrations/*.sql` in `.gitignore`). Its first deploy (11:57 UTC) still did nothing: the deploy's migration step ran `psql -U $(printenv POSTGRES_USER)`, the postgres container has no `POSTGRES_USER`, and psql failed with `role "-d" does not exist` behind `|| true`, so no migration had ever run. The step now runs [`ops/backend/run-migrations.sh`](../../ops/backend/run-migrations.sh), which uses the first superuser that connects (`POSTGRES_USER`, then the image default `postgres`, then `DB_USER`) and fails the step when a migration fails. It runs whenever `backend/migrations/` or that script changes, or when the Deploy workflow is started by hand, with the password from the `CSDM_ANALYZER_PASSWORD` GitHub secret, which must equal the password in the VM settings file. The deploy log of the "Run database migrations" step prints `csdm_analyzer: role and privileges applied` and `csdm_schema_version`. |
...
| Analyzer worker with upload support on the game VM | The first run on `main` (26 September 2026) started the VM, then spent the whole 45-minute job on direct SSH attempts that the firewall drops, and was cancelled. The VM kept the 11 September worker, which ignores the signed link and fetches every archived demo with `gcloud storage cp` as the VM's own account, so the first uploaded demo failed with `archive download failed ... 123719540575-compute@developer.gserviceaccount.com`. The setup script now uses IAP first with time limits. The 10:40 UTC dry run got `4033: 'not authorized'` from IAP; after the owner granted `backend-ci` `roles/iap.tunnelResourceAccessor`, the 11:21 UTC run on `main` installed the worker. The next analysis then got as far as running `csdm` and failed inside its database migration check, which a missing or data-only `csdm_analyzer` role both cause (row above). That cancelled run also left the VM on outside any analysis session, so nothing closes it automatically: close it from the Demolar page when you are done. |
...
- The analyzer role reads and writes CS Demo Manager's own tables only (`migrations` read-only) plus `stats_refresh_state`, which the website's stats triggers write with the inserting role's rights; the website's tables in the same database (`admins`, `steam_members`, ...) are out of its reach. The CLI runs `CREATE TABLE IF NOT EXISTS migrations` before every command, and Postgres demands CREATE on the schema for that even when the table exists, so the role has CREATE on `public`; the `csdm_analyzer_no_ddl` event trigger rolls back anything that statement or any other would actually create, and the role owns no tables, so the CLI can never migrate or reset the schema from the game VM. Because of that the CLI version (3.20.1) and the desktop version you push official stats with must stay equal, and production must already be at the 3.20.1 schema (version **14**, printed as `csdm_schema_version` in the migration step's CI log). If it is lower, connect your desktop CS Demo Manager 3.20.1 to production once, as you do when pushing stats, so it migrates before any analysis runs. If it is higher (the desktop app was updated), the CLI refuses with "Database schema version mismatch"; install the matching CLI version on the game VM and extend the table list in the SQL file.
- The migration is idempotent (it rebuilds the role's privileges on every run) and only reports the schema version when the secret is absent. It was tested on PostgreSQL 16 with the website's real stats trigger: the CLI's startup statements (schema-version read, `CREATE TABLE IF NOT EXISTS migrations` in a transaction), inserts, `\copy` into a bigserial table and a cascading `--force` delete succeed; reading or writing `admins`, writing `migrations`, creating tables, views or functions (including a `public.lower(varchar)` shadow), `SELECT INTO`, `ALTER`/`DROP TABLE` and CS Demo Manager's `DROP SCHEMA public` reset are all refused.
...
`archive download failed: ... uploads/<name>.dem' (or it may not exist) ... authenticated as <number>-compute@developer.gserviceaccount.com` on an uploaded demo means the game VM runs a worker without upload support (it ignores the signed link and asks the VM's own account, which cannot read `uploads/`). Run the Demo analysis setup workflow on `main` (Actions > Demo analysis setup > Run workflow), check that it ends green with "Installing worker" or "Worker already up to date", then press "Analiz et" again. `csdm exited 1: Error: getaddrinfo ENOTFOUND ...` means the database hostname in the CLI settings does not resolve. On 26 September 2026 every value in that hand-written file ended in a Windows carriage return (`hostname='10.142.0.9\r'`), so no analysis could ever connect; the setup workflow now writes the file and both writers strip such whitespace. Run Demo analysis setup on `main` and check for "Database login OK".

`csdm exited 1: Error while migrating database` followed by `permission denied for schema public`, `permission denied for table ...` or `password authentication failed for user "csdm_analyzer"` means the database role is missing or out of date: run the Deploy workflow by hand (it always runs the migrations then) and check the migration step's log. The worker now drops stack frames from what it reports, so the website shows the actual message; the full CLI output is in `journalctl -u csbatagi-analyzer`.

The current worker reports a missing link as "the backend sent no signed download link" instead; that one points to the backend log line `[demos] worker link failed`.
